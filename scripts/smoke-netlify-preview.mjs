...
if (!rawBaseUrl) {
  console.error(
    "Provide a base URL as the first argument or set SMOKE_BASE_URL.",
  );
  process.exit(1);
}
...
function assertStatus(name, result, allowedStatuses) {
  if (!allowedStatuses.includes(result.status)) {
    throw new Error(
      `${name} returned ${result.status}. Expected one of: ${allowedStatuses.join(", ")}`,
    );
  }
}
...
function assertJsonLike(name, result) {
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    throw new Error(
      `${name} did not return JSON. Content-Type was: ${contentType || "unknown"}`,
    );
  }
}
...
async function main() {
  ...
  const sharedSummary = await request(
    "/.netlify/functions/sharedSummary?topic=test&category=news",
  );
  assertStatus("sharedSummary", sharedSummary, [200, 404, 503]);
  assertJsonLike("sharedSummary", sharedSummary);

  const generatedContent = await request(
    "/.netlify/functions/generatedContent?type=news&topic=technology&count=1",
  );
  ...
  const sources = await request("/.netlify/functions/sources");
  assertStatus("sources", sources, [200]);
  assertJsonLike("sources", sources);
  const sourcesPayload = JSON.parse(sources.text);
  if (!Array.isArray(sourcesPayload?.sources)) {
    throw new Error("sources response did not include a sources array.");
  }
