...
const REQUIRED = [
  {
    ...
    reason:
      "Auth session tokens should be hashed with a deployment-specific pepper.",
  },
];
...
const RECOMMENDED = [
  ...
  },
  {
    key: "RSS_APP_API_KEY",
    reason:
      "Allows server-side authenticated RSS.app feed requests when your provider expects an API key header.",
  },
  {
    key: "RSS_APP_API_SECRET",
    reason:
      "Allows server-side authenticated RSS.app feed requests when your provider expects an API secret header.",
  },
  {
    key: "NEWS_API_KEY",
    reason:
      "Enables the fetchNews serverless endpoint for NewsAPI-backed content.",
  },
  {
    key: "GNEWS_API_KEY",
    reason:
      "Enables the fetchNews serverless endpoint for GNews-backed content.",
  },
  {
    key: "SOCIAL_RSS_FEEDS",
    reason:
      "Lets you override baked-in social feed definitions per environment.",
  },
  {
...
const AI_GROUPS = [
  ...
  },
  {
    ...
    reason:
      "Without this, server-side perspective labeling falls back to heuristic classification only.",
  },
];
...
function validateAnthropicConfig(messages) {
  ...
  if (serverKey) {
    if (looksLikePlaceholder(serverKey)) {
      messages.errors.push(
        "ANTHROPIC_API_KEY looks like a placeholder. Set a real server-side Anthropic key.",
      );
    }

    if (!serverKey.startsWith("sk-ant-")) {
      messages.warnings.push(
        "ANTHROPIC_API_KEY does not start with the expected Anthropic prefix. Double-check for typos or an outdated key.",
      );
    }

    if (/\s/.test(serverKey)) {
      messages.errors.push(
        "ANTHROPIC_API_KEY contains whitespace. Paste the key as a single uninterrupted value.",
      );
    }
  }
...
function validateBlobPair(messages) {
  ...
  if (hasSiteID !== hasToken) {
    messages.errors.push(
      "Set both NETLIFY_BLOBS_SITE_ID and NETLIFY_BLOBS_TOKEN together for Docker/CI/manual runtimes.",
    );
  } else if (hasSiteID && hasToken) {
    messages.info.push(
      "Manual Netlify Blobs credentials detected for non-Netlify runtime support.",
    );
  } else {
    messages.info.push(
      "Manual Blob credentials not set. This is fine on Netlify; Docker/CI/manual runtimes will use graceful Blob fallbacks, but story persistence, rate-limit buckets, and generated-content caching will be partial or disabled.",
    );
  }
}
...
function main() {
  ...
  if (requireBlobs) {
    if (
      !hasValue("NETLIFY_BLOBS_SITE_ID") ||
      !hasValue("NETLIFY_BLOBS_TOKEN")
    ) {
      messages.errors.push(
        "Blob credentials are required in this mode. Set NETLIFY_BLOBS_SITE_ID and NETLIFY_BLOBS_TOKEN to avoid local/CI Blob warnings and disabled persistence.",
      );
    }
  }
