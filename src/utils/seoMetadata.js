...
const nodeSiteUrl = globalThis?.process?.env?.VITE_SITE_URL || "";

export const SITE_URL = viteSiteUrl || nodeSiteUrl || "https://thelatest.com";
export const DEFAULT_DESCRIPTION =
  "Breaking news, politics, tech, business, culture, video, and podcast coverage curated for fast, high-signal reading.";
...
function getStoryMetadata(search = "") {
  ...
  return {
    ...
    description:
      article.description ||
      (article.source
        ? `Read coverage from ${article.source} on ${SITE_NAME}.`
        : DEFAULT_DESCRIPTION),
...
export function buildRouteMetadata(locationLike = {}, options = {}) {
  ...
  if (pathname === "/") {
    ...
    description =
      "The topics, sources, and categories you follow on The Latest.";
    robots = "noindex,nofollow";
  } else if (pathname === "/article") {
    title = `Article Reader | ${SITE_NAME}`;
    description =
      "Read the full story in The Latest article reader with fast on-site access and related coverage.";
    robots = "noindex,nofollow";
  } else if (pathname === "/privacy") {
    title = `Privacy Policy | ${SITE_NAME}`;
    description =
      "How The Latest collects, uses, stores, and protects account, reading, and analytics information.";
    ...
    description =
      "What The Latest is building, how it approaches news discovery, and what it prioritizes in product trust.";
  } else if (pathname === "/advertise") {
    title = `Advertise | ${SITE_NAME}`;
    description =
      "Advertising, sponsorship, and branded placement information for campaigns on The Latest.";
  } else if (pathname === "/editorial-standards") {
    title = `Editorial Standards | ${SITE_NAME}`;
    description =
      "How The Latest handles source selection, generated fallback content, labeling, and corrections.";
  } else if (pathname === "/corrections") {
    title = `Corrections | ${SITE_NAME}`;
    description =
      "How to report factual, routing, or labeling issues and how The Latest handles corrections.";
  } else if (pathname === "/contact") {
    title = `Contact | ${SITE_NAME}`;
    description =
      "Contact The Latest for editorial questions, product support, corrections, and privacy requests.";
    ...
    const sectionLabel = topicConfig
      ? getTopicSectionLabel(segments[1])
      : "Topics";
    ...
    if (segments[2] === "all-news") {
      ...
      description =
        ...
        `${topicLabel} coverage, context, and the latest developments from The Latest.`;
    }
  } else {
    const routeMap = {
      news: {
        ...
        description:
          "The latest reporting, headlines, and developing stories from around the world.",
      },
      opinions: {
        title: `Opinions | ${SITE_NAME}`,
        description:
          "Opinion, commentary, and analysis spanning politics, business, culture, and technology.",
      },
      videos: {
        title: `Videos | ${SITE_NAME}`,
        description:
          "Watch breaking-news clips, explainers, interviews, and curated video coverage.",
      },
      podcasts: {
        title: `Podcasts | ${SITE_NAME}`,
        description:
          "Listen to interviews, daily briefings, and podcast coverage from across the news cycle.",
      },
      social: {
        title: `Social Media | ${SITE_NAME}`,
        description:
          "Trending social posts and conversation snapshots connected to the biggest stories.",
      },
      sports: {
        title: `Sports | ${SITE_NAME}`,
        description:
          "Scores, highlights, and the top sports stories from around the globe.",
      },
      "all-news": {
        title: `All News | ${SITE_NAME}`,
        description:
          "A broader stream of live headlines, top stories, and fast-moving updates from The Latest.",
      },
      "all-opinions": {
        title: `All Opinions | ${SITE_NAME}`,
        description:
          "A deeper feed of commentary, columns, and opinion coverage from The Latest.",
      },
      "all-videos": {
        title: `All Videos | ${SITE_NAME}`,
        description:
          "A broader stream of video reporting, highlights, and visual explainers from The Latest.",
      },
      "all-podcasts": {
        title: `All Podcasts | ${SITE_NAME}`,
        description:
          "A broader stream of podcasts, interviews, and audio briefings from The Latest.",
      },
    };
...
export function buildStructuredData(locationLike = {}, metadata = {}) {
  ...
  if (pathname === "/") {
    ...
    breadcrumbItems.push({
      name: config.title,
      item: `${baseUrl}/category/${segments[1]}`,
    });
    ...
    const sectionLabel = topicConfig
      ? getTopicSectionLabel(segments[1])
      : "Topics";
    const sectionTarget = topicConfig?.feedCategory
      ? `${baseUrl}/category/${topicConfig.feedCategory}`
      : `${baseUrl}/news`;
    pageType = "CollectionPage";
    breadcrumbItems.push({ name: sectionLabel, item: sectionTarget });
    breadcrumbItems.push({
      name: topicLabel,
      item: `${baseUrl}/topic/${segments[1]}`,
    });
  ...
  if (segments[0] === "story" && segments[1]) {
    ...
    if (storyMeta?.article) {
      graph.push({
        ...
        articleSection:
          storyMeta.article.category || storyMeta.article.source || "News",
      });
    }
  }
