...
const formatHostnameOutlet = (hostname) => {
  ...
  const suffixMatch = Object.keys(DOMAIN_MAP).find(
...
const humanizeHostnameOutlet = (hostname) => {
  const normalized = String(hostname || "")
    .replace(/^www\./, "")
    .toLowerCase();
  if (!normalized) return "";
...
export const deriveMediaOutlet = (itemOrSource, maybeUrl) => {
  const source =
    typeof itemOrSource === "object" && itemOrSource !== null
      ? itemOrSource.source
      : itemOrSource;
  const url =
    typeof itemOrSource === "object" && itemOrSource !== null
  ...
  try {
    ...
    if (cleanedSource && !looksLikeByline(cleanedSource)) {
      return cleanedSource;
    }

    const fallbackOutlet = humanizeHostnameOutlet(hostname);
    if (fallbackOutlet) return fallbackOutlet;
  } catch {
