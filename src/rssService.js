...
const RETRY_ATTEMPTS = 2; // allow one recovery attempt before falling back to stale/generated content
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter(
          (key) =>
            typeof key === "string" &&
            key.startsWith(`${RSS_CACHE_VERSION}_`) &&
            !key.includes("_search_"),
        )
      : [];
  } catch {
    return [];
...
const registerBucketKey = (cacheKey) => {
  ...
  try {
    ...
    window.localStorage.setItem(
      RSS_DISCOVERY_INDEX_KEY,
      JSON.stringify([...currentKeys, cacheKey]),
    );
  } catch {
    ...
    try {
      ...
      if (isHtmlLikeResponse(response)) {
        const invalidPayloadError = new Error(
          `Invalid RSS payload received for ${url}`,
        );
      ...
      console.debug(
        `[RSS] Retry ${attempt}/${attempts} for ${url} in ${delay}ms`,
      );
      await sleep(delay);
    }
  }
...
const fetchFeedWithCacheFallback = async ({
  ...
  if (!canAttemptFunctions()) {
    if (staleData.length > 0) {
      console.log(
        `[RSS] Skipping ${label} fetch - functions unavailable, using stale cache`,
      );
      return staleData;
    }

    console.log(
      `[RSS] ${label} has no stale cache; bypassing temporary function cooldown for a fresh attempt`,
    );
  }
  ...
  if (staleData.length > 0 && timeoutMs > 0) {
    ...
    if (timedResult.kind === "error") {
      console.error(
        `[RSS] Error fetching ${label}:`,
        timedResult.error.message,
      );
    ...
    console.warn(
      `[RSS] Returning stale ${label} cache for ${cacheKey} after ${timeoutMs}ms soft timeout`,
    );
    return staleData;
  }
...
const evaluateSearchRelevance = (item, searchTerm) => {
  ...
  const descriptionHits = countTokenHits(
    tokenizeSearchText(item.description),
    queryTokens,
  );
  const contentHits = countTokenHits(
    tokenizeSearchText(item.content),
    queryTokens,
  );
  const metaHits = countTokenHits(
    tokenizeSearchText(`${item.source || ""} ${item.category || ""}`),
    queryTokens,
  );
  ...
  const totalMatched =
    titleHits.matched +
    descriptionHits.matched +
    contentHits.matched +
    metaHits.matched;
  const requiredCoverage =
    queryTokens.length === 1 ? 1 : Math.ceil(queryTokens.length * 0.6);
  const titleOnlyWeak =
    titleHits.score > 0 && bodyScore === 0 && metaHits.score === 0;

  return {
    isMatch:
      totalMatched >= requiredCoverage &&
      weightedScore >= 7.5 &&
      !titleOnlyWeak,
    score: weightedScore,
  };
  ...
  try {
    ...
    const cacheKeys =
      registeredKeys.length > 0
        ? registeredKeys
        : (await cacheManager.getAllKeys()).filter(
            (key) =>
              typeof key === "string" &&
              key.startsWith(`${RSS_CACHE_VERSION}_`) &&
              !key.includes("_search_"),
    ...
    const matchedItems = cachedItems.filter(
      (item) => evaluateSearchRelevance(item, searchTerm).isMatch,
    );
...
const dedupeContentItems = (items = []) => {
  ...
  return items.filter((item) => {
    ...
    const titleKey = normalizedTitle
      ? `title:${normalizedSource}|${normalizedTitle}`
      : "";
    ...
    if (
      (normalizedUrl && seen.has(`url:${normalizedUrl}`)) ||
...
export async function fetchRSSNews(category = null) {
  ...
  if (normalizedCategory) {
    ...
    console.log(
      `[RSS] Fetching news with category filter: ${normalizedCategory}`,
    );
  }
  ...
  const articles = await fetchFeedWithCacheFallback({
    ...
    transform: (payload) => {
      ...
      if (dedupedArticles.length !== normalizedArticles.length) {
        console.log(
          `[RSS] Removed ${normalizedArticles.length - dedupedArticles.length} duplicate news items`,
        );
      }
  ...
  console.log(
    `[RSS] Resolved ${articles.length} news articles${normalizedCategory ? ` for ${normalizedCategory}` : ""}`,
  );
  return articles;
}
...
export async function fetchRSSOpinions(category = null) {
  ...
  if (normalizedCategory) {
    params.append("category", normalizedCategory);
    console.log(
      `[RSS] Fetching opinions with category filter: ${normalizedCategory}`,
    );
  }
  ...
  const opinions = await fetchFeedWithCacheFallback({
    ...
    transform: (payload) => {
      ...
      if (dedupedOpinions.length !== normalizedOpinions.length) {
        console.log(
          `[RSS] Removed ${normalizedOpinions.length - dedupedOpinions.length} duplicate opinion items`,
        );
      }
  ...
  console.log(
    `[RSS] Resolved ${opinions.length} opinion pieces${normalizedCategory ? ` for ${normalizedCategory}` : ""}`,
  );
  return opinions;
}
...
export async function fetchRSSVideos(category = null) {
  ...
  if (normalizedCategory) {
    params.append("category", normalizedCategory);
    console.log(
      `[RSS] Fetching videos with category filter: ${normalizedCategory}`,
    );
  }
  ...
  const videos = await fetchFeedWithCacheFallback({
    ...
    transform: (payload) => {
      ...
      if (dedupedVideos.length !== normalizedVideos.length) {
        console.log(
          `[RSS] Removed ${normalizedVideos.length - dedupedVideos.length} duplicate video items`,
        );
      }
  ...
  console.log(
    `[RSS] Resolved ${videos.length} videos${normalizedCategory ? ` for ${normalizedCategory}` : ""}`,
  );
  return videos;
}
...
export async function fetchRSSPodcasts(category = null) {
  ...
  if (normalizedCategory) {
    params.append("category", normalizedCategory);
    console.log(
      `[RSS] Fetching podcasts with category filter: ${normalizedCategory}`,
    );
  }
  ...
  const podcasts = await fetchFeedWithCacheFallback({
    ...
    transform: (payload) => {
      ...
      if (dedupedPodcasts.length !== normalizedPodcasts.length) {
        console.log(
          `[RSS] Removed ${normalizedPodcasts.length - dedupedPodcasts.length} duplicate podcast items`,
        );
      }
  ...
  console.log(
    `[RSS] Resolved ${podcasts.length} podcasts${normalizedCategory ? ` for ${normalizedCategory}` : ""}`,
  );
  return podcasts;
}
...
export async function searchRSSContent(searchTerm, options = {}) {
  ...
  const maxBackendWaitMs = Number.isNaN(parsedRemoteWait)
    ? 0
    : Math.max(0, parsedRemoteWait);
  const parsedMinStrict = Number.parseInt(options.minStrictResults, 10);
  const minStrictResults = Number.isNaN(parsedMinStrict)
    ? 6
    : Math.max(1, Math.min(parsedMinStrict, 20));
  ...
  try {
    ...
    if (cachedSearch && cachedSearch.length > 0) {
      console.log(
        `[RSS Search] Cache hit for "${normalizedSearchTerm}" with ${cachedSearch.length} results`,
      );
      if (fastLocalOnly) {
        return cachedSearch;
      }
    ...
    const localResults =
      cachedSearch && cachedSearch.length > 0
        ? cachedSearch
    ...
    if (preferFresh) {
      ...
      console.log(
        `[RSS Search] Prefer-fresh remote search for "${searchTerm}" at ${freshUrl}`,
      );
      ...
      try {
        ...
        const normalizedFreshResults = extractDataArray(response).map(
          normalizeOutletSource,
        );
        ...
        console.warn(
          "[RSS Search] Prefer-fresh remote search failed:",
          error.message,
        );
        if (shouldDisableFunctions(error)) {
          markFunctionsUnavailable();
    ...
    if (localResults.length > 0) {
      console.log(
        `[RSS Search] Local cache hit for "${searchTerm}" with ${localResults.length} results`,
      );
      ...
      const remotePromise = (async () => {
        ...
        console.log(
          `[RSS Search] Deep searching backend for "${searchTerm}" at ${url}`,
        );
        const response = await requestWithRetry(url);
        markFunctionsAvailable();

        const normalizedResults = extractDataArray(response).map(
          normalizeOutletSource,
        );
      ...
      if (Array.isArray(remoteResults) && remoteResults.length > 0) {
        ...
        console.log(
          `[RSS Search] Returning merged local+remote results: ${merged.length}`,
        );
        return merged;
      }
    ...
    if (!canAttemptFunctions()) {
      console.warn(
        "[RSS Search] Functions unavailable - backend not accessible",
      );
      return [];
    }
    ...
    const remoteSearchPromise = (async () => {
      ...
      console.log(
        `[RSS Search] After extraction: ${normalizedResults.length} items`,
      );
    ...
    if (!remoteOutcome) {
      console.warn(
        `[RSS Search] Backend search exceeded ${maxBackendWaitMs || SEARCH_REMOTE_COLD_TIMEOUT_MS}ms for "${searchTerm}"; falling back to local results`,
      );
      return [];
    }
    ...
    if (results.length > 0) {
      ...
      if (results.length !== normalizedCount) {
        console.log(
          `[RSS Search] Removed ${normalizedCount - results.length} duplicates`,
        );
      }
      console.log(
        `[RSS Search] ✓ Found ${results.length} results for "${searchTerm}"`,
      );
      return results;
    }

    console.warn(
      `[RSS Search] No results found for "${searchTerm}" from backend`,
    );
    return [];
  } catch (error) {
