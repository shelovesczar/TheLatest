...
function extractTopicsFromHeadlines(articles, topN = 12) {
  ...
  articles.forEach((article) => {
    ...
    tokens.forEach((token) => {
      const isProper =
        /^[A-Z]/.test(token) && !/^\d+$/.test(token) && !stopWords.has(token);
      if (isProper) {
...
const normalizeRailTopic = (value = "") =>
  String(value || "")
    .replace(/\s+/g, " ")
    .trim();
...
const areTopicListsEqual = (left = [], right = []) => {
...
const itemKey = (item) => {
  ...
  const fallbackTitle = String(item?.title || "")
    .trim()
    .toLowerCase();
  const fallbackSource = String(item?.source || "")
    .trim()
    .toLowerCase();
...
const splitMediaWithoutOverlap = (videoItems, podcastItems) => {
  ...
  const uniquePodcasts = dedupeContentItems(podcastItems || []).filter(
    (item) => {
...
const ensureTopicCoverage = (
  ...
  const topicMatchedBackup = backup.filter((item) =>
    matchesTopicQuery(item, topicValue),
  );
  const merged = dedupeContentItems([...primary, ...topicMatchedBackup]);

  if (merged.length < minItems) {
    console.log(
      `[HomePage] Topic "${topicValue}" has limited matches (${merged.length}/${minItems})`,
    );
  }
...
function HomePage({ onBreakingNewsChange }) {
  ...
  const handleSummaryDataChange = useCallback((nextSummary) => {
    const nextTopics = sanitizeRailTopics(nextSummary?.suggestedTopics || []);

    setSummaryTopics((currentTopics) =>
      areTopicListsEqual(currentTopics, nextTopics) ? currentTopics : nextTopics,
    );
  }, []);
  ...
  const shouldRenderOpinionsSection = hasActiveTopic || opinionsInView || opinions.length > 0;
  const shouldRenderVideosSection = hasActiveTopic || videosInView || videos.length > 0;
  const shouldRenderPodcastsSection =
    hasActiveTopic || podcastsInView || podcasts.length > 0;
  const shouldRenderSocialSection = hasActiveTopic || socialInView || socialPosts.length > 0;
  const shouldShowOpinionsSection = loadingOpinions || opinions.length > 0;
  ...
  useEffect(() => {
    const extractedTopics = sanitizeRailTopics(
      extractTopicsFromHeadlines(topStories, 12),
    );
  ...
  useEffect(() => {
    ...
    const loadInitialContent = async () => {
      ...
      try {
        console.log(
          "Loading initial content, hasActiveTopic:",
          hasActiveTopic,
          "topic:",
          topic,
        );
        ...
        if (hasActiveTopic && topic && topic.trim().length > 0) {
          ...
          console.log(
            `[HomePage] Search returned ${searchResults.length} results`,
          );
          ...
          const [
            ...
            fetchRSSNews(),
            fetchRSSVideos(),
            fetchStoryClusters({ type: "news", search: topic, limit: 8 }),
          ]);

          const opinionsPool =
            opinionsPoolResult.status === "fulfilled"
              ? opinionsPoolResult.value || []
              : [];
          const videosPool =
            videosPoolResult.status === "fulfilled"
              ? videosPoolResult.value || []
              : [];
          const podcastsPool =
            podcastsPoolResult.status === "fulfilled"
              ? podcastsPoolResult.value || []
              : [];
          const newsPool =
            newsPoolResult.status === "fulfilled"
              ? newsPoolResult.value || []
              : [];
          const rssVideosPool =
            rssVideoPoolResult.status === "fulfilled"
              ? rssVideoPoolResult.value || []
              : [];
          const nextStoryClusters =
            storyClusterResult.status === "fulfilled"
              ? storyClusterResult.value || []
              : [];
          ...
          const guaranteedNews = ensureTopicCoverage(
            categorizedResults.news,
            newsPool,
            topic,
            MIN_TOPIC_NEWS,
          );
          const guaranteedOpinions = ensureTopicCoverage(
            categorizedResults.opinions,
            opinionsPool,
            topic,
            MIN_TOPIC_OPINIONS,
          );
          ...
          const guaranteedPodcasts = ensureTopicCoverage(
            categorizedResults.podcasts,
            podcastsPool,
            topic,
            MIN_TOPIC_PODCASTS,
          );
          ...
          const { uniqueVideos, uniquePodcasts } = splitMediaWithoutOverlap(
            guaranteedVideos,
            guaranteedPodcasts,
          );
          ...
          setStoryClusters(Array.isArray(nextStoryClusters) ? nextStoryClusters : []);
          ...
          const [newsResult, storyClusterResult] = await Promise.allSettled([
            fetchRSSNews(),
      ...
      try {
        if (!hasActiveTopic) {
          ...
          setPodcasts((currentPodcasts) => {
            const { uniquePodcasts } = splitMediaWithoutOverlap(
              nextVideos,
              currentPodcasts || [],
            );
  ...
  useEffect(() => {
    const loadSocial = async () => {
      const socialQuery = hasActiveTopic
        ? (topic || "").trim().toLowerCase()
        : "__all__";
      if (!socialInView) return;
      if (socialPosts.length > 0 && lastSocialQueryRef.current === socialQuery)
        return;

      setLoadingSocial(true);
      try {
        const socialData = await getRandomTrendingPosts(
          12,
    ...
    <main className="main-content home-main-content">
      ...
      <div className="page-body">
        ...
        <div ref={opinionsRef} data-section="opinions">
          {shouldRenderOpinionsSection && shouldShowOpinionsSection ? (
            <Suspense fallback={<SectionLoader />}>
              <Opinions opinions={opinions} loadingOpinions={loadingOpinions} />
            </Suspense>
          ) : null}
        </div>
        ...
        <div ref={videosRef} data-section="videos">
          {shouldRenderVideosSection && shouldShowVideosSection ? (
            <Suspense fallback={<SectionLoader />}>
              <Videos videos={videos} loadingVideos={loadingVideos} />
            </Suspense>
          ) : null}
        </div>
        ...
        <div ref={podcastsRef} data-section="podcasts">
          {shouldRenderPodcastsSection && shouldShowPodcastsSection ? (
            <Suspense fallback={<SectionLoader />}>
              <Podcasts podcasts={podcasts} loadingPodcasts={loadingPodcasts} />
            </Suspense>
          ) : null}
        </div>
        ...
        <div ref={socialRef} data-section="social">
          {shouldRenderSocialSection && shouldShowSocialSection ? (
            <Suspense fallback={<SectionLoader />}>
              <SocialMedia
