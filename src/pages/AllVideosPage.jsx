...
function AllVideosPage({ category = null }) {
  ...
  const loadVideos = useCallback(async () => {
    ...
    try {
      ...
      if (activeTopicQuery && activeTopicQuery.trim().length > 0) {
        ...
        let topicVideos = dedupeByMediaKey(
          removeCrossDuplicates(searchVideos, searchPodcasts),
        );
  ...
  const fallbackTo = topicRoute || categoryRoute || "/videos";
  const breadcrumbs = [
    { label: "Home", to: "/" },
    topicRoute
  ...
  return (
    <div className="all-news-page">
      <PageBackBar
        ...
        meta={`${filteredVideos.length} videos in this feed`}
      />
      ...
      <div className="all-news-content">
        {loading ? (
          ...
          <div className="no-results">
            <p>
              No videos found{filterContext ? ` for "${filterContext}"` : ""}.
            </p>
          </div>
        ) : (
          <>
                  ...
                  <div className="lead-story-content">
                    ...
                    <div className="lead-story-footer">
                      <span className="lead-story-source">
                        {leadStory.duration || leadStory.source}
                      </span>
                      <a
                        href={leadStoryHref}
                        target="_blank"
                ...
                {featuredStories.map((item, index) => (
                  ...
                  >
                    {item.image && (
                      ...
                      >
                        <img
                          {...getImageProps(item.image, item.title, "videos")}
                        />
                      </a>
                    )}
