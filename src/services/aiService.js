...
const hasAnyBrowserAiKeyConfigured = () =>
  Object.values(CONFIG.apiKeys).some((value) => cleanText(value));
...
const getCategoryThemeTerms = (items = [], categorySlug = "", limit = 3) => {
  ...
  return (
    CATEGORY_THEME_DEFAULTS[categorySlug] ||
    CATEGORY_THEME_DEFAULTS["top-stories"]
  ).slice(0, limit);
};
...
const buildThemePhrase = (terms = []) => {
  ...
  if (cleanTerms.length === 2)
    return `${cleanTerms[0]} and ${cleanTerms[1]} in Focus`;
...
async function generateEditorialSummary(topic = "", context = {}) {
  ...
  if (hasCategoryScope) {
    const [newsByCategory, opinionsByCategory, videosByCategory] =
    ...
    categoryPool = [
      ...(newsByCategory.status === "fulfilled"
        ? newsByCategory.value || []
        : []),
      ...(opinionsByCategory.status === "fulfilled"
        ? opinionsByCategory.value || []
        : []),
      ...(videosByCategory.status === "fulfilled"
        ? videosByCategory.value || []
        : []),
    ];

    const strictScoped = filterToCategoryScope(
      [...pool, ...categoryPool],
      normalizedCategory,
      true,
    );
  ...
  if (!Array.isArray(pool) || pool.length === 0) {
    ...
    if (hasCategoryScope) {
      ...
      const looseScoped = filterToCategoryScope(
        pool,
        normalizedCategory,
        false,
      );
      pool =
        strictScoped.length > 0
          ? strictScoped
          : looseScoped.length > 0
            ? looseScoped
            : pool;
    }
  }
  ...
  if (hasCategoryScope) {
    const scopedStrict = filterToCategoryScope(
      unique,
      normalizedCategory,
      true,
    );
    const scopedLoose = filterToCategoryScope(
      unique,
      normalizedCategory,
      false,
    );
    const scopedUnique =
      scopedStrict.length > 0
        ? scopedStrict
        : scopedLoose.length > 0
          ? scopedLoose
          : [];
    ...
    if (scopedUnique.length === 0 && categoryPool.length > 0) {
      ...
      const prioritizedScoped = prioritizeCategoryItems(
        scopedUnique,
        normalizedCategory,
      );
      ...
      if (prioritizedScoped.length > 0) {
        ...
        unique.push(
          ...[...scopedUnique].sort(
            (a, b) => scoreItemFreshness(b) - scoreItemFreshness(a),
          ),
        );
      }
    }
  ...
  const theme =
    terms.length > 0 ? terms.join(", ") : "policy, markets, and culture";

  const sourceCount = new Set(
    ranked.map((item) => String(item?.source || "").trim()).filter(Boolean),
  ).size;
  const sourcePreview = [
    ...new Set(
      ranked.map((item) => String(item?.source || "").trim()).filter(Boolean),
  ...
  const intro = normalizedTopic
    ...
    : toSentence(
        `Today's biggest moves center on ${theme}: ${trimHeadline(lead?.title || "")}`,
      );

  const follow = second
    ? toSentence(
        `Also in focus, ${trimHeadline(second?.title || "")}${third ? `, while ${trimHeadline(third?.title || "")}` : ""}`,
      )
    : "";
  ...
  const headlineTopic =
    normalizedTopic ||
    (hasCategoryScope ? normalizedCategory.replace(/-/g, " ") : "");
  const headline = buildEditorialHeadline(headlineTopic, lead, terms);
  const image =
    lead?.image || lead?.thumbnail || second?.image || second?.thumbnail || "";
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a news analyst providing concise, factual summaries of current events. Focus on the most recent developments and keep summaries to exactly 150 words.",
          },
          {
            role: "user",
            content: prompt,
...
export async function generateAISummary(requestOrTopic = "", options = {}) {
  ...
  if (
    CONFIG.provider === AI_PROVIDERS.EDITORIAL ||
    !hasAnyBrowserAiKeyConfigured()
  ) {
    const editorialOnly = await generateEditorialSummary(
      effectiveQuery,
      request,
    );
  ...
  const editorialSummary = await generateEditorialSummary(
    effectiveQuery,
    request,
  );
...
export async function getSharedSummary(requestOrTopic = "", options = {}) {
  ...
  try {
    const response = await fetch(
      `${SHARED_SUMMARY_ENDPOINT}?${params.toString()}`,
    );
...
export async function getSearchAssist(query = "") {
  ...
  try {
    ...
    const response = await fetch(
      `${SEARCH_ASSIST_ENDPOINT}?${params.toString()}`,
    );
    if (!response.ok) return null;
...
export async function persistSharedSummary(
  requestOrTopic = "",
  summaryData,
  options = {},
) {
