...
const SUMMARY_ADS = [
  {
    ...
    secondaryCopy:
      "Tax-aware investing plans with guided allocations for long-horizon savers.",
    ...
    image:
      "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=300&h=180&fit=crop",
  ...
  },
  {
    ...
    secondaryCopy:
      "Built for product teams that need performance without adding ops drag.",
    ...
    image:
      "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=300&h=180&fit=crop",
  ...
  },
  {
    ...
    secondaryCopy:
      "Designed for quick planning when you want a trip to feel considered, not rushed.",
    ...
    image:
      "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?w=300&h=180&fit=crop",
...
const getSummaryAdDensity = (height = 0) => {
  if (height >= 440) return "expanded";
  if (height >= 320) return "balanced";
  return "compact";
};

const buildCompactSummaryText = (
  text = "",
  maxSentences = 2,
  maxChars = 460,
) => {
  ...
  const safeClip =
    lastSpace > maxChars * 0.65 ? clipped.slice(0, lastSpace) : clipped;
  return `${safeClip.replace(/[,:;-]+$/, "")}...`;
};

const getDynamicSummaryLineHeight = (textLength = 0) => {
  if (textLength > 360) return "1.42";
  if (textLength > 220) return "1.46";
  return "1.5";
};
...
const buildSummaryNote = (summaryData) => {
  ...
  if (sources.length === 0) {
    return {
      prefix: "Briefing based on current reporting signals.",
      sourcesText: "",
      suffix: " Verify fast-moving details with primary reporting.",
    };
  }

  return {
    prefix: "Briefing based on reporting signals from ",
    sourcesText: formatSourceList(sources),
    suffix: ".",
  };
};
...
function AISummary({
  ...
  useEffect(() => {
    if (typeof onSummaryDataChange === "function") {
  ...
  const summaryLineHeight = useMemo(
    () => getDynamicSummaryLineHeight(summaryBodyText.length),
    [summaryBodyText],
  );
  const providerLabel = summaryData?.provider || "AI Summary";
  const providerDisplay = providerLabel.startsWith("Claude")
    ? "Claude by Anthropic"
    : providerLabel;
  const briefingLabel = useTopicFilter
    ...
    : category === "general"
      ? "TOP STORIES"
      : formatCategoryLabel().toUpperCase();
  const leadInLabel = useTopicFilter
    ? `${topic}:`
    : category === "general"
      ? "Today's Top Stories: "
      : `Today's ${formatCategoryLabel()}`;
  const noteLabel = buildSummaryNote(summaryData);
  const updateLabel = isRefreshing
    ? "Refreshing briefing..."
  ...
  return (
    <section
      className={`section ai-summary-section ai-summary-section--${layout}`}
    >
        ...
        <article className="summary-card" ref={summaryCardRef}>
          <div className="summary-body">
            <div className="summary-kicker-row">
              ...
              <div className="summary-kicker-text">
                AI DAILY BRIEFING — {briefingLabel}
              </div>
            ...
            <div className="summary-note">
              {noteLabel.prefix}
              {noteLabel.sourcesText && (
                <span className="summary-note-source">
                  {noteLabel.sourcesText}
                </span>
              )}
              {noteLabel.suffix}
            ...
            <div className="summary-footer">
              ...
              >
                <span className="summary-provider-label">
          ...
          >
            ...
            <div className="summary-ad-body">
              <div className="summary-ad-main">
                ...
                <div className="summary-ad-copy summary-ad-copy-secondary">
                  {activeAd.secondaryCopy}
                </div>
              </div>
              ...
              <div className="summary-ad-footer-wrap">
                ...
                <div className="summary-ad-disclaimer">
                  {activeAd.disclaimer}
                </div>
              </div>
            </div>
