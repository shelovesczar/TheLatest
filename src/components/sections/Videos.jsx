...
function Videos({ loadingVideos, videos, categoryPath }) {
  ...
  const getVideoSourceLabel = (video) => {
    ...
    const publisher =
      author && !/unknown|staff/i.test(author) ? author : outlet;
  ...
  const prevSlide = () => {
    ...
    setCurrentIndex((prev) => {
      if (prev === 0) {
        ...
        const lastPageStart =
          Math.floor((safeVideos.length - 1) / itemsPerPage) * itemsPerPage;
      ...
      {loadingVideos ? (
        ...
        <>
                ...
                return (
                  ...
                  >
                    <div className="card-thumb video-thumbnail">
                      <img
                        {...getImageProps(video.image, video.title, "videos")}
                      />
                      ...
                      <div
                        className={`play-button${isHovered && canPreview ? " is-hidden" : ""}`}
                        aria-hidden="true"
                      >
                    ...
                    <div className="card-body-inner">
                      <div className="card-source-row">
                        ...
                        {getGeneratedContentLabel(video) && (
                          <span className="card-date">
                            {getGeneratedContentLabel(video)}
                          </span>
                        )}
                        <span className="card-date">
                          {formatDateOnly(video.date)}
                        </span>
                      </div>
          ...
          <div className="section-see-more-row">
            <Link to={categoryPath || "/all-videos"} className="see-more">
              See more videos →
            </Link>
          </div>
        </>
