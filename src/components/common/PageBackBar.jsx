...
function PageBackBar({
  ...
  return (
    <div
      className="page-back-bar"
      style={{ "--page-back-bar-offset": `${stickyOffset}px` }}
    >
      <div className={`page-back-bar__shell ${shellClassName}`.trim()}>
        <div className="page-back-bar__inner">
          <div className="page-back-bar__leading">
            <button
              type="button"
              className="page-back-bar__button"
              onClick={handleBack}
              aria-label="Go back"
            >
            ...
            <nav className="page-back-bar__breadcrumbs" aria-label="Breadcrumb">
              <ol className="page-back-bar__crumb-list">
                {breadcrumbs.map((crumb, index) => (
                  <li
                    className="page-back-bar__crumb"
                    key={`${crumb.label}-${index}`}
                  >
                    {index > 0 ? (
                      <span className="page-back-bar__separator">/</span>
                    ) : null}
                    {crumb.to ? (
