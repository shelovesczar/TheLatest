...
const resolvePerspective = ({
  ...
  const explicitKey = safeStr(perspectiveKey).trim().toLowerCase();
  const explicitLabel = safeStr(perspectiveLabel).trim();
  const explicitMethod = safeStr(perspectiveMethod).trim().toLowerCase();

  if (explicitKey && explicitKey !== "unknown") {
    return {
      key: explicitKey,
      label: explicitLabel || "Unclassified",
      method: explicitMethod || "source-map",
    };
  }
...
const NewsCard = ({
  ...
  const trust = getTrustDescriptor(safeSource);
  ...
  return (
    ...
    >
      <div className="news-card-image-container">
        ...
        {safeCategory && <div className="news-card-badge">{safeCategory}</div>}
      </div>
      ...
      <div className="news-card-content">
        <div className="news-card-meta">
          ...
          {perspective ? (
            <span
              className={`news-card-perspective news-card-perspective--${perspective.key}`}
              title={
                METHOD_LABELS[perspective.method] || METHOD_LABELS["source-map"]
              }
            >
              {perspective.label}
            </span>
