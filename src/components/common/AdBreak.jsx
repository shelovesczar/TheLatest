...
const AD_CAMPAIGNS = [
  {
    ...
    support:
      "Treasury yield, card controls, and clean finance workflows in one place.",
  ...
  },
  {
    ...
    support:
      "Private itineraries, editorial guides, and member-only hotel perks across every major city.",
  ...
  },
  {
    ...
    support:
      "Charge faster, route smarter, and keep every cabin detail exactly where you want it.",
  ...
  },
  {
    ...
    support:
      "Sleep, recovery, training, and nutrition recommendations that stay readable and actionable.",
...
const getCreativeVariant = ({
  ...
  if (Number.isInteger(campaignIndex)) {
    return AD_CAMPAIGNS[
      ((campaignIndex % AD_CAMPAIGNS.length) + AD_CAMPAIGNS.length) %
        AD_CAMPAIGNS.length
    ];
  }
  ...
  if (sponsor) {
    return {
      ...
      support:
        "This branded placeholder keeps the new ad treatment while reserving space for live creative.",
      badge: sizeLabel,
      cta: "Visit Sponsor",
    };
  }
...
const AdCreative = ({ creative, type, sizeLabel }) => (
  <div
    className={`ad-creative ${type === "sidebar" ? "sidebar" : "leaderboard"}`}
  >
      ...
      {type === "sidebar" && (
        <div className="ad-creative-points">
          {creative.points.map((point) => (
            <span key={point} className="ad-creative-point">
              {point}
            </span>
          ))}
        </div>
...
const AdBreak = ({
  ...
  const useStaticCreative =
    Number.isInteger(campaignIndex) || sponsor || children;
  ...
  const content = children ? (
    <div className="ad-live-slot">{children}</div>
  ) : (
    <AdCreative
      creative={creative}
      type={adConfig.type}
  ...
  if (adConfig.type === "sidebar") {
    return (
      <div
        className="ad-break sidebar"
        style={style}
        data-slot={slot || adConfig.type}
      >
  ...
  return (
    <div
      className={`ad-break ${adConfig.type}`}
      style={style}
      data-slot={slot || adConfig.type}
    >
      <div className="ad-label">{adConfig.label}</div>
      <div className="ad-container">{content}</div>
    </div>
  );
};
