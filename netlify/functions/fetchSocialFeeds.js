...
const {
  socialFeeds: localSocialFeeds = [],
} = require("./social-feed-config.cjs");
...
const parseEnvFeeds = () => {
  try {
    ...
    console.error(
      "[SocialFeeds] Invalid SOCIAL_RSS_FEEDS JSON:",
      error.message,
    );
    return [];
  }
...
const resolveFallbackUrl = (feed) => {
  if (feed?.fallbackUrl && /^https?:\/\//i.test(feed.fallbackUrl))
    return feed.fallbackUrl;
  return null;
};
...
const getMaxItemsForFeed = (feed) => {
  ...
  return (
    feed.maxItems ||
    feedConfig.fetchConfig.defaultItemsPerFeed * (feed.weight || 1)
  );
};
...
const scoreFeedRelevance = (feed = {}, topic = "") => {
  const query = String(topic || "")
    .trim()
    .toLowerCase();
  if (!query) return 0;
  ...
  const sourceText = [feed?.source, feed?.category, feed?.platform]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
...
const matchesTopic = (post, topic) => {
  ...
  const metaHits = countHits(
    tokenize(
      `${post?.author || ""} ${post?.source || ""} ${post?.category || ""} ${(post?.tags || []).join(" ")}`,
    ),
    queryTokens,
  );

  const weightedScore =
    titleHits.score * 6 + bodyHits.score * 4 + metaHits.score * 1.5;
  const hasCoverage =
    titleHits.matched + bodyHits.matched + metaHits.matched >=
    Math.max(1, Math.ceil(tokenize(query).length * 0.6));
  const titleOnlyWeakMatch =
    titleHits.score > 0 && bodyHits.score === 0 && metaHits.score === 0;
  ...
  if (activeFeeds.length === 0) {
    return {
      ...
      body: JSON.stringify({
        ...
        configuredFeeds: allFeeds.length,
        warning:
          "No active feeds in current rotation cycle. Check social-feed-config.cjs for configuration.",
      }),
    };
  }
        ...
        for (const candidateUrl of urlsToTry) {
          try {
            const response = await axios.get(candidateUrl, {
              timeout: 20000,
              responseType: "text",
              headers: buildFeedRequestHeaders(candidateUrl),
            });
            parsed = await parser.parseString(response.data);
            if (Array.isArray(parsed?.items) && parsed.items.length > 0) break;
          } catch (error) {
            console.warn(
              `[SocialFeeds] Failed feed ${feed.source} via ${candidateUrl}: ${error.message}`,
            );
          }
        }
