...
const {
  getManagedSources,
  buildManagedSourceKey,
} = require("./rss-aggregator.cjs");
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === "GET") {
    const feedKey = String(event.queryStringParameters?.feedKey || "")
      .trim()
      .toLowerCase();
  ...
  if (!sourceId || active === null) {
    return {
      ...
      headers,
      body: JSON.stringify({
        error: "sourceId and boolean active are required.",
      }),
    };
  }
  ...
  const updated = {
    ...
    url: target.url,
    active,
    ownershipName: String(body.ownershipName || "").trim() || null,
    ownershipType: String(body.ownershipType || "").trim() || null,
    perspectiveKey:
      String(body.perspectiveKey || "")
        .trim()
        .toLowerCase() || null,
    perspectiveLabel: String(body.perspectiveLabel || "").trim() || null,
    trustScore: Number.isFinite(Number(body.trustScore))
  ...
  await setJson(
    STORE_NAMES.sources,
    buildManagedSourceKey(target.id),
    updated,
    {
      metadata: {
        feedKey: target.feedKey,
        ...
        provider: String(target.provider || "rss").slice(0, 40),
  ...
  return {
    ...
    headers,
    body: JSON.stringify({
      source: { ...target, ...updated, updatedAt: updated.updatedAt },
    }),
  };
};
