...
const DEFAULT_IMAGE_BY_CATEGORY = {
  politics:
    "https://images.unsplash.com/photo-1529107386315-e1a2ed48a620?w=1200&h=700&fit=crop",
  technology:
    "https://images.unsplash.com/photo-1677442136019-21780ecad995?w=1200&h=700&fit=crop",
  business:
    "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=1200&h=700&fit=crop",
  entertainment:
    "https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=1200&h=700&fit=crop",
  sports:
  ...
  science:
    "https://images.unsplash.com/photo-1516849677043-ef67c9557e16?w=1200&h=700&fit=crop",
  climate:
    "https://images.unsplash.com/photo-1569163139394-de4798aa62b6?w=1200&h=700&fit=crop",
  environment:
    "https://images.unsplash.com/photo-1569163139394-de4798aa62b6?w=1200&h=700&fit=crop",
  general:
    "https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=1200&h=700&fit=crop",
};
...
function parseRequest(event = {}) {
  ...
  const type = ["news", "opinions", "videos", "podcasts"].includes(
    requestedType,
  )
    ? requestedType
    : "news";
...
async function getCoverageItems({ type, topic, category }) {
  const requestSets = [{ type, ...(category ? { category } : {}) }];
  ...
  const settled = await Promise.allSettled(
    requestSets.map((params) => fetchAggregatorItems(params)),
  );
  const merged = [];
  const seen = new Set();
...
function buildPrompt({ type, topic, category, count, items }) {
  ...
  const scope = topic ? `the topic "${topic}"` : `${categoryLabel} coverage`;

  const typeInstructions = {
    news: "Create concise news briefings that read like original aggregation copy, not copied source articles.",
    opinions:
      "Create clearly labeled analysis or opinion-style briefings with a point of view, but avoid pretending to be a real columnist or publication.",
    videos:
      "Create watch-list style video briefing cards that summarize what a viewer would expect to learn from an on-site explainer.",
    podcasts:
      "Create listen-list style podcast briefing cards that summarize what a listener would expect to hear in an on-site audio-style explainer.",
  };
  ...
  return (Array.isArray(rawItems) ? rawItems : [])
    ...
    .map((item, index) => {
      const title = truncateText(
        item?.title ||
          `${toTitleCase(request.topic || request.category || request.type)} briefing`,
        110,
      );
      const description = truncateText(
        item?.description || item?.content || `${title} from The Latest.`,
        180,
      );
      ...
      return {
        ...
        source: cleanText(
          item?.sourceLabel ||
            fallbackSourceByType[request.type] ||
            "The Latest Claude Briefing",
        ),
        author: cleanText(
          item?.author || item?.sourceLabel || "The Latest Desk",
        ),
        hosts: cleanText(item?.hosts || item?.author || "The Latest Desk"),
        duration: cleanText(
          item?.duration ||
            (request.type === "videos"
              ? `${4 + (index % 4)}:${index % 2 === 0 ? "12" : "48"}`
              : ""),
        ),
        category: cleanText(
          item?.categoryLabel || request.topic || categoryLabel || "News",
        ),
        type:
          request.type === "opinions"
            ? "opinion"
            : request.type === "videos"
              ? "video"
...
async function readCachedRequestPayload(key = "") {
  try {
    ...
    if (!isBlobConfigurationError(error)) {
      console.warn(
        "[generatedContent] failed to read cached request payload:",
        error.message,
      );
    }
    return null;
  ...
  try {
    ...
    await Promise.allSettled(
      items.map(async (item) => {
        await setJson(
          STORE_NAMES.generated,
          buildItemKey(item.generatedId),
          item,
        );
    ...
    if (!isBlobConfigurationError(error)) {
      console.warn(
        "[generatedContent] failed to cache generated payload:",
        error.message,
      );
    }
  }
...
async function readGeneratedItem(id = "") {
  ...
  try {
    ...
    if (!isBlobConfigurationError(error)) {
      console.warn(
        "[generatedContent] failed to read cached generated item:",
        error.message,
      );
    }
    return null;
...
async function generateWithAnthropic(request = {}) {
  ...
  const model =
    getConfigValue("ANTHROPIC_CONTENT_FALLBACK_MODEL") ||
...
function buildStaticFallbackItems(request = {}) {
  const topicLabel = cleanText(
    request.topic || toTitleCase(request.category || request.type || "news"),
  );
  ...
  const rawItems = (baseByType[request.type] || baseByType.news).map(
    (title, index) => ({
      title,
...
async function resolveGeneratedItems(request = {}) {
  ...
  if (
    cached &&
    isFresh(cached.timestamp) &&
    cached.provider !== "Editorial fallback"
  ) {
    return cached;
  }
...
async function generateAndCachePayload(request = {}) {
  ...
  const items =
    generated.items.length > 0
      ? generated.items
      : buildStaticFallbackItems(request);
  const payload = {
    request,
...
function scheduleGeneratedRefresh(request = {}) {
  ...
  const refreshPromise = generateAndCachePayload(request)
    .catch((error) => {
      console.warn(
        "[generatedContent] background refresh failed:",
