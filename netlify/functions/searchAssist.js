...
const SEARCH_ASSIST_TTL_MS = 30 * 60 * 1000;
const SEARCH_ASSIST_STALE_TTL_MS = 6 * 60 * 60 * 1000;
...
function buildSearchAssistKey(query = "") {
  return `query/${normalizePart(query) || "general"}`;
}

function isFresh(timestamp, ttlMs = SEARCH_ASSIST_TTL_MS) {
...
function sanitizeShortList(
  values = [],
  { min = 2, max = 40, maxWords = 5, limit = 6 } = {},
) {
  const cleaned = [];
  const seen = new Set();
...
function buildPrompt(query = "", items = []) {
  ...
  return [
    ...
    ...(hasItems ? ["", "Article notes:", buildCoverageContext(items)] : []),
  ].join("\n");
}
...
function buildFallbackSearchAssist(query = "") {
  ...
  return {
    normalizedQuery,
    topicBrief: truncateText(
      `Related search angles and coverage paths for ${normalizedQuery}.`,
      140,
    ),
...
async function tryPersistSearchAssist(key, payload) {
  try {
    await setJson(STORE_NAMES.searchAssist, key, payload, {
      metadata: {
        query: normalizePart(payload.normalizedQuery) || "general",
        provider: String(payload.provider || "Search Assist").slice(0, 80),
      },
    });
  } catch (error) {
...
async function generateAnthropicSearchAssist(query = "") {
  ...
  try {
    ...
    return {
      ...
      topicBrief: truncateText(
        parsed?.topicBrief || fallbackData.topicBrief,
        140,
      ),
      suggestedTopics:
        suggestedTopics.length > 0
          ? suggestedTopics
          : fallbackData.suggestedTopics,
      searchPhrases:
        searchPhrases.length > 0 ? searchPhrases : fallbackData.searchPhrases,
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const key = buildSearchAssistKey(query);
    let cached = null;

    try {
      cached = await getJsonWithMetadata(STORE_NAMES.searchAssist, key);
    } catch (error) {
      if (!isBlobConfigurationError(error)) {
        throw error;
      }
    ...
    if (cached?.data && isFresh(cached.data.timestamp, SEARCH_ASSIST_TTL_MS)) {
      return {
        statusCode: 200,
        headers: { ...headers, ...rateLimit.headers },
        body: JSON.stringify({
    ...
    if (data) {
      await tryPersistSearchAssist(key, data);
    }

    if (
      !data &&
      cached?.data &&
      isFresh(cached.data.timestamp, SEARCH_ASSIST_STALE_TTL_MS)
    ) {
      return {
        statusCode: 200,
        headers: { ...headers, ...rateLimit.headers },
